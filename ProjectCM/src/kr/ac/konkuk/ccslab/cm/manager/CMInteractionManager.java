...
public class CMInteractionManager {
	...
	private static boolean processLOGIN(CMMessage msg, CMInfo cmInfo)
	{
		...
		if(confInfo.getSystemType().equals("SERVER"))
		{
			...
			else
			{
				...
				//팀플 추가
				cmInfo.getConfigurationInfo().setTestMessage("변경후 1");
				System.out.println("제발 출력돼라");
	...
	public static boolean replyToLOGIN(CMSessionEvent se, int nValidUser, CMInfo cmInfo)
	{
		...
		//팀플
//		seAck.m_sessionMessage = "야야야야야야야";
		...
		cmInfo.getConfigurationInfo().setTestMessage("변경후 - replyToLogin");
		System.out.println("reply 변경 후 Sysout");
		...
		if(nValidUser == 1)
		{
			...
			//팀플 추가
			cmInfo.getConfigurationInfo().setTestMessage("변경 후 -replyToLogin");
	...
	private static void distributeLoginUsers(String targetUser, CMInfo cmInfo)
	{
		...
		while(iterUser.hasNext())
		{
			...
			if(!targetUser.equals(loginUser.getName()))
			{
				...
				//팀플 추가
				cmInfo.getConfigurationInfo().setTestMessage("변경 후 -distribute");
	...
	private static void processLOGIN_ACK(CMMessage msg, CMInfo cmInfo)
	{
		...
		if(CMInfo._CM_DEBUG)
		{
			System.out.println("~~~~~~~~~~~~~~~~"+se.m_sessionMessage);
	...
	private static void processSESSION_ADD_USER(CMMessage msg, CMInfo cmInfo)
	{
		...
		if(confInfo.getSystemType().equals("CLIENT"))
		{
			if(CMInfo._CM_DEBUG)
			{
				...
				System.out.println("SESSION_ADD_USER : "+cmInfo.getConfigurationInfo().getTestMessage());
...
//        due.setSender(strMyName);
	...
	private static void processRES_SERVER_REG(CMMultiServerEvent mse, CMInfo cmInfo)
	{
		...
		if(mse.getReturnCode() == 1)
		{
			...
			{
				//팀플 추가
				cmInfo.getConfigurationInfo().setTestMessage("변경 후 - processRES_SERVER_REG");
	...
	public static boolean replyToADD_LOGIN(CMMultiServerEvent mse, int nValidUser, CMInfo cmInfo)
	{
		...
		if(nValidUser == 1)
		{
			...
			//팀플 추가
			cmInfo.getConfigurationInfo().setTestMessage("변경 후 -replyToAdd_login");
	...
	private static void distributeAddLoginUsers(String strUser, CMInfo cmInfo)
	{
		...
		while(iter.hasNext())
		{
			...
			if(!strUser.equals(tuser.getName()))
			{
				...
				//팀플 추가
				cmInfo.getConfigurationInfo().setTestMessage("변경 후 -distributeAdd_login");
